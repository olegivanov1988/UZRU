...
void InitTIMER3 () {
  ...
  MDR_TIMER3->PSG = 0;    // Значение предварительного делителя счётчика. Основной счётчик считает на частоте CLK = TIM_CLK/(PSG+1)
  
  MDR_TIMER3->ARR = 0xFFFFFFFF;

  MDR_TIMER3->CNTRL = 
  ( 10 << 8 ) |  // EVENT_SEL - Биты выбора источника событий ( 0000 - TIM_CLK, 0001 - CNT==ARR в таймере 1, 0010 - CNT==ARR в таймере 2, 0011 - CNT==ARR в таймере 3, 0100 - событие на первом канале, ..., 1000 - событие переднего фронта ETR, 1001 - событие заднего фронта ETR, 1010 - CNT==ARR в таймере 4 )
  ( 2 << 6 ) |  // CNT_MODE - Режим счёта основного счётчика ( 00 - счётчик вверх при DIR==0 и счётчик вниз при DIR==1 при PSG==0, 01 - счётчик вверх/вниз с автоматическим изменением DIR при CNT==0 или CNT==ARR, 1x - счётчик вверх при DIR==0 и счётчик вниз при DIR==1 при EVENT==1 )
  ...
  MDR_TIMER3->CCR1 = 0; // CCR - Значение CCR, с которым сравнивается CNT при работе в ШИМ-режиме. Значение CNT, при котором произошёл факт захвата события в режиме захвата.
  
  MDR_TIMER3->CCR11 = 0; // CCR1 - Значение, с которым сравнивается CNT при работе в ШИМ-режиме. Значение CNT, при котором произошёл факт захвата события в режиме захвата.
  
  MDR_TIMER3->CH1_CNTRL =
