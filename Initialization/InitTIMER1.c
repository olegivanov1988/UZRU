...
void InitTIMER1 () {
  ...
  MDR_TIMER1->PSG = 0;    // Значение предварительного делителя счётчика. Основной счётчик считает на частоте CLK = TIM_CLK/(PSG+1)
  
  MDR_TIMER1->ARR = 2 * MAX_DISCHARGE_SIGNAL;

  MDR_TIMER1->CNTRL = 
  ( 0 << 8 ) |  // EVENT_SEL - Биты выбора источника событий ( 0000 - TIM_CLK, 0001 - CNT==ARR в таймере 1, 0010 - CNT==ARR в таймере 2, 0011 - CNT==ARR в таймере 3, 0100 - событие на первом канале, ..., 1000 - событие переднего фронта ETR, 1001 - событие заднего фронта ETR, 1010 - CNT==ARR в таймере 4 )
  ...
  ( 1 << 1 ) |  // ARRB_EN - Разрешение мгновенного обновления ARR ( 0 - ARR будет перезаписан в момент записи в ARR, 1 - ARR будет перезаписан при завершении счёта CNT )
  ( 1 << 0 );    // CNT_EN - Разрешение работы таймера ( 0 - таймер отключен, 1 - таймер включен )

  
  // Скважность ШИМ 1 мкс
	
  MDR_TIMER1->CCR1 = 0; // CCR - Значение CCR, с которым сравнивается CNT при работе в ШИМ-режиме. Значение CNT, при котором произошёл факт захвата события в режиме захвата.
  MDR_TIMER1->CCR2 = MAX_DISCHARGE_SIGNAL;
	
	MDR_TIMER1->CCR11 = 0;
	MDR_TIMER1->CCR21 = 0;
	
  //MDR_TIMER1->CCR11 = 1600; // CCR1 - Значение, с которым сравнивается CNT при работе в ШИМ-режиме. Значение CNT, при котором произошёл факт захвата события в режиме захвата.
  ...
  ( 6 << 9 )  | // OCCM - Формат выработки сигнала REF в режиме ШИМ ( см. техническое описание )
  ...
  ( 3 << 10 ) | // NSELO - Режим работы выхода инверсного канала ( 00 - всегда на выход выдаётся 0, канал на выходе не работает, 01 - всегда на выход выдаётся 1, канал всегда работает на выход, 10 - на выход выдаётся сигнал REF, 11 - на выход выдаётся сигнал DTG )
  ( 1 << 8 )  | // NSELOE - Режим работы инверсного канала на выход ( 00 - всегда на OE выдаётся 0, канал на выход не работает, 01 - всегда на OE выдаётся 1, канал всегда работает на выход, 10 - на OE выдаётся сигнал REF, при REF==0 вход, при REF==1 выход, 11 - на OE выдаётся сигнал с DTG, при CHn==0 вход, при CHn==1 выход )
  ( 0 << 4 )  | // INV - Режим выходной инверсии прямого канала ( 0 - выход не инвертируется, 1 - выход инвертируется )
  ( 3 << 2 )  | // SELO - Режим работы выхода прямого канала ( 00 - всегда на выход выдаётся 0, канал на выходе не работает, 01 - всегда на выход выдаётся 1, канал всегда работает на выход, 10 - на выход выдаётся сигнал REF, 11 - на выход выдаётся сигнал DTG )
  ( 1 << 0 )  ; // SELOE - Режим работы прямого канала на выход ( 00 - всегда на OE выдаётся 0, канал на выходе не работает, 01 - всегда на OE выдаётся 1, канал всегда работает на выход, 10 - на OE выдаётся сигнал REF, при REF==0 вход, при REF==1 выход. 11 - на OE выдаётся сигнал DTG, при CH==0 вход, при CH==1 выход )

  MDR_TIMER1->CH1_CNTRL2 =
  ( 1 << 3 ) | // CRRRLD - Разрешение обновления регистров CCR и CCR1 ( 0 - обновление возможно в любой момент времени, 1 - обновление будет осуществлено только при CNT=0 )
  ( 1 << 2 ) | // CCR1_EN - Разрешение работы регистра CCR1 ( 0 - CCR1 не используется, 1 - CCR1 используется )
  ...
  MDR_TIMER1->CH1_DTG = 
  ( 255 << 8 ) | // DTGx - Основной делитель частоты ( от 0 до 256, задержка DTGdel==DTGx*(DTG+1) )
  ( 1 << 4 )  | // EDTS - Выбор источника тактовой частоты для DTG ( 0 - TIM_CLK, 1 - FDTS )
  ( 0 << 0 )  ; // DTG - Предварительный делитель частоты

  MDR_TIMER1->CH2_CNTRL =
  ( 0 << 15 ) | // CAP_nPWM - Ржим работы канала Захват или ШИМ ( 0 - канал работает в режиме ШИМ, 1 - канал работает в режиме Захват )
  ( 0 << 14 ) | // WR_CMPL - Флаг окончания записи, при задании нового значения регистра CCR ( 0 - новые данные можно записывать, 1 - данные не записаны и идёт запись )
  ( 0 << 13 ) | // ETREN - Разрешение сброса по выводу ETR ( 0 - запрещен сброс, 1 - разрешен сброс )
  ( 0 << 12 ) | // BRKEN - Разрешение сброса по выводу BRK ( 0 - запрещен сброс, 1 - разрешен сброс )
  ...
  MDR_TIMER1->CH3_CNTRL =
  ( 0 << 15 ) | // CAP_nPWM - Ржим работы канала Захват или ШИМ ( 0 - канал работает в режиме ШИМ, 1 - канал работает в режиме Захват )
  ( 0 << 14 ) | // WR_CMPL - Флаг окончания записи, при задании нового значения регистра CCR ( 0 - новые данные можно записывать, 1 - данные не записаны и идёт запись )
  ( 1 << 13 ) | // ETREN - Разрешение сброса по выводу ETR ( 0 - запрещен сброс, 1 - разрешен сброс )
  ( 1 << 12 ) | // BRKEN - Разрешение сброса по выводу BRK ( 0 - запрещен сброс, 1 - разрешен сброс )
  ...
  MDR_TIMER1->CH3_DTG = 
  ( 96 << 8 ) | // DTGx - Основной делитель частоты ( от 0 до 256, задержка DTGdel==DTGx*(DTG+1) )
  ( 0 << 4 )  | // EDTS - Выбор источника тактовой частоты для DTG ( 0 - TIM_CLK, 1 - FDTS )
  ( 0 << 0 )  ; // DTG - Предварительный делитель частоты

  MDR_TIMER1->CH4_CNTRL =
  ( 0 << 15 ) | // CAP_nPWM - Ржим работы канала Захват или ШИМ ( 0 - канал работает в режиме ШИМ, 1 - канал работает в режиме Захват )
  ( 0 << 14 ) | // WR_CMPL - Флаг окончания записи, при задании нового значения регистра CCR ( 0 - новые данные можно записывать, 1 - данные не записаны и идёт запись )
  ( 1 << 13 ) | // ETREN - Разрешение сброса по выводу ETR ( 0 - запрещен сброс, 1 - разрешен сброс )
  ( 1 << 12 ) | // BRKEN - Разрешение сброса по выводу BRK ( 0 - запрещен сброс, 1 - разрешен сброс )
