...
void InitTIMER4 () {
  ...
  MDR_TIMER4->PSG = 0;    // Значение предварительного делителя счётчика. Основной счётчик считает на частоте CLK = TIM_CLK/(PSG+1)
  
  MDR_TIMER4->ARR = 0xFFFFFFFF;

  MDR_TIMER4->CNTRL = 
  ( 0 << 8 ) |  // EVENT_SEL - Биты выбора источника событий ( 0000 - всегда "0", 0001 - CNT==ARR в таймере 1, 0010 - CNT==ARR в таймере 2, 0011 - CNT==ARR в таймере 3, 0100 - событие на первом канале, ..., 1000 - событие переднего фронта ETR, 1001 - событие заднего фронта ETR, 1010 - CNT==ARR в таймере 4 )
  ...
  MDR_TIMER4->CCR1 = 0; // CCR - Значение CCR, с которым сравнивается CNT при работе в ШИМ-режиме. Значение CNT, при котором произошёл факт захвата события в режиме захвата.
  
  MDR_TIMER4->CCR11 = 0; // CCR1 - Значение, с которым сравнивается CNT при работе в ШИМ-режиме. Значение CNT, при котором произошёл факт захвата события в режиме захвата.
  
  MDR_TIMER4->CH1_CNTRL =
